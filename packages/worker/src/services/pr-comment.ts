...
export function buildInlineComment(finding: RawFinding): string {
  ...
  const withAutoFix = finding as RawFinding & {
    autoFix?: { suggestedCode: string; confidence: number };
  };

  if (withAutoFix.autoFix && withAutoFix.autoFix.confidence >= 0.4) {
    lines.push('');
    lines.push('**Suggested fix:**');
    lines.push('```suggestion');
    lines.push(withAutoFix.autoFix.suggestedCode);
    lines.push('```');
    lines.push('');
    lines.push(`> 🤖 Auto-fix generated by CodeSheriff (confidence: ${Math.round(withAutoFix.autoFix.confidence * 100)}%)`);
  }
